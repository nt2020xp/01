...
def channel_list():
    ...
    if 'id' not in request.args:
        ...
        for channel_id, data in channels.items():
            response.append(f"{data[0]},{base_url}?id={channel_id}")
        return Response('\n'.join(response), mimetype='text/plain; charset=utf-8')
    
    channel_id = request.args.get('id')
    if channel_id not in channels:
        response = ["頻道不存在，可用頻道列表："]
        for cid, cdata in channels.items():
            response.append(f"{cdata[0]},{base_url}?id={cid}")
        return Response('\n'.join(response), mimetype='text/plain; charset=utf-8'), 404
